...
@property (nonatomic, strong) NSMutableArray*totalLayersArray;
...
- (void)commonInit
{
    ...
    _totalLayersArray = [NSMutableArray array];
...
- (void)setDefaultParameters
{
    ...
    _indexLabelFont = [UIFont systemFontOfSize:10];
...
-(void)setYTitleMutableArray:(NSMutableArray *)yTitleMutableArray{
    _yTitleMutableArray = yTitleMutableArray;
}
...
//    CGFloat minBound = [self minVerticalBound];
    ...
    NSString* text;  //= _yTitleMutableArray[index];

    text = _labelForValue((maxBound / 5) * (index + 1));
    if (maxBound<15) {
        text = _labelForValue((15 / 5) * (index));
        
    }
    if (index ==0) {
        text = _labelForValue((15 / 5) * 0);
    ...
    if (index ==0) {
        ...
        label.frame = CGRectMake(p.x - xOffset,p.y+13, width + 2, 14);
    }
    ...
    if (index>0 && index<5) {
        ...
        CGFloat labelHeight = ((mainRect.size.height-170)/5)*(index);
        if (index ==1) {
            label.frame = CGRectMake(p.x -xOffset,p.y- labelHeight+15, width + 2, 14);
        }else if (index ==2){
            label.frame = CGRectMake(p.x - xOffset, p.y-labelHeight+10, width + 2, 14);
        }else if (index ==3){
            label.frame = CGRectMake(p.x - xOffset, p.y-labelHeight, width + 2, 14);
        }else if (index ==4){
            label.frame = CGRectMake(p.x - xOffset-2, p.y-labelHeight-10, width + 2, 14);
        }
...
-(void)setCurrentTimeIndex:(NSInteger )currentTimeIndex{
...
- (UILabel*)createLabelForIndex: (NSUInteger)index
{
    ...
    float width = [text boundingRectWithSize:rect.size
                                  ...
                                  context:nil].size.width;
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - 30.0f, p.y + 12, width + 2, 14)];
    
    if (_currentTimeIndex ==0) {
        if (index ==0) {
            label.frame = CGRectMake(p.x - 23.0f, p.y + 12, width + 2, 14);
            
        }
        
    }
        if (_currentTimeIndex ==4) {
            label.frame =CGRectMake(p.x - 44.0f, p.y + 18, width + 2, 14);
    
        }
    if (_currentTimeIndex ==3) {
        
            label.frame = CGRectMake(p.x - 20.0f, p.y + 12, width + 2, 14);
        if (index ==0) {
            label.frame = CGRectMake(p.x - 16.0f, p.y + 12, width + 2, 14);
            
        }
        
        
    }
    if (_currentTimeIndex ==6) {
        if (index ==0) {
    ...
    if (_currentTimeIndex ==2 || _currentTimeIndex ==1) {
       // label.transform = CGAffineTransformRotate(label.transform, M_PI_4);
        label.frame = CGRectMake(p.x - 7.0f, p.y + 12, width + 2, 14);
        if (index ==7) {
    ...
    if (_currentTimeIndex ==5) {
      //  label.transform = CGAffineTransformRotate(label.transform, M_PI_4);
        label.frame = CGRectMake(p.x - 20.0f, p.y + 12, width + 2, 14);
        if (index ==0) {
            label.frame = CGRectMake(p.x - 10.0f, p.y + 12, width + 2, 14);
            
        }
        if (index ==6) {
            label.frame = CGRectMake(p.x - 30.0f, p.y + 12, width + 2, 14);
    ...
    if(_drawInnerGrid) {
        for(int i=0;i<_horizontalGridStep;i++) {
            ...
            CGContextMoveToPoint(ctx, point.x - 0.5f, _axisHeight + _margin);
              #warning 改动 CGContextAddLineToPoint(ctx, point.x - 0.5f, _axisHeight + _margin + 3);
...
- (void)strokeChart
{
    ...
    fill.lineCapStyle = kCGLineCapRound;
    fill.lineJoinStyle = kCGLineJoinRound;
    if(_fillColor) {
        ...
        if (fillLayer.frame.origin.y<0) {
            CGRect rect = fillLayer.frame;
            rect.origin.x =0;
...
- (void)strokeDataPoints
{
    ...
    for(int i=0;i<_data.count;i++) {
        ...
        fillLayers = [CAShapeLayer layer];
        fillLayers.frame = CGRectMake(p.x, p.y, _dataPointRadius, _dataPointRadius);
        fillLayers.bounds = CGRectMake(p.x, p.y, _dataPointRadius, _dataPointRadius);
        fillLayers.path = circle.CGPath;
        fillLayers.strokeColor = _dataPointColor.CGColor;
        fillLayers.fillColor = _dataPointBackgroundColor.CGColor;
        
        fillLayers.lineWidth = 1;
        fillLayers.lineJoin = kCALineJoinRound;
        
        [self.layer addSublayer:fillLayers];
        [self.layers addObject:fillLayers];
        [self.totalLayersArray addObject:fillLayers];
        UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(fillLayers.frame.origin.x-25*OffWidth, fillLayers.frame.origin.y-10*OffHeight, fillLayers.frame.size.width+40*OffWidth, fillLayers.frame.size.height+10*OffHeight);
        //btn.backgroundColor = [UIColor redColor];
        btn.titleEdgeInsets = UIEdgeInsetsMake(0*OffHeight, btn.titleLabel.bounds.size.width, 20*OffHeight, 5*OffHeight);
...
-(void)btnClick:(UIButton *)sender{
    for (id  tempObj in  self.subviews) {
        if ([tempObj isKindOfClass:[UIButton class]]) {
            UIButton *tempBtn=tempObj;
            [tempBtn setTitleColor:[UIColor clearColor] forState:UIControlStateNormal];
    ...
    if (selectLayer) {
        ...
        [selectLayer addAnimation:animation forKey:@"scale-layer"];
    }
    
    UIButton * btn = [self viewWithTag:sender.tag];
    [sender setTitle:_data[sender.tag] forState:UIControlStateNormal];
    [sender setTitleColor: [UIColor blueColor] forState:UIControlStateNormal];
    sender.titleLabel.font = [UIFont systemFontOfSize:14];
    
    CAShapeLayer * layerS = [self.totalLayersArray objectAtIndex:_totalLayersArray.count/2 +sender.tag];
    ...
    keyAnimation.values = @[@1.5,@1.3,@0.7,@1.2,@1.5];
    keyAnimation.fillMode = kCAFillModeForwards;
    keyAnimation.removedOnCompletion = NO;
    [layerS addAnimation:keyAnimation forKey:@"pointScale"];
...
{
    return MAX(_YMaxRange, 0);
}
-(void)setYMaxRange:(CGFloat)YMaxRange{
    _YMaxRange = YMaxRange;
}
- (void)computeBounds
{
    _min = MAXFLOAT;
...
{
    _verticalGridStep = gridStep;//垂直坐标
    _horizontalGridStep = gridStep;//水平坐标
}
...
- (UIBezierPath*)getLinePath:(float)scale withSmoothing:(BOOL)smoothed close:(BOOL)closed
{
    ...
    if(smoothed) {
        for(int i=0;i<_data.count - 1;i++) {
            ...
            if (controlPoint[1].x<0) {
                controlPoint[1].x =0;
            }
            controlPoint[1].y = p.y - m.y * _bezierSmoothingTension;
            if (controlPoint[1].y<0) {
                controlPoint[1].y =0;
                
            }
            
            if(![_data[i] isEqualToString:@"0"] || i == 0 || ![_data[i+1] isEqualToString:@"0"]) {
            
                [path addCurveToPoint:p controlPoint1:controlPoint[0] controlPoint2:controlPoint[1]];//三次曲线
                
            }else {
            
                [path addLineToPoint:[self getPointForIndex:i+1 withScale:scale]];
